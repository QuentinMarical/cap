...
if (not file.IsDir("stargate/keyboard","DATA")) then
	file.CreateDir("stargate/keyboard");
end
...
function StarGate.KeyBoard:New(name)
	...
	t.__Keys = {};
	for k,v in pairs(util.KeyValuesToTable(file.Read("stargate/keyboard/"..name:lower()..".txt","DATA") or "")) do -- Load keysettings
...
function StarGate.KeyBoard:SetKey(name,key)
	...
	file.Write("stargate/keyboard/"..self.Name:lower()..".txt",util.TableToKeyValues(self.__Keys or {}));
end
